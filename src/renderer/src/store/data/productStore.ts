...
interface State {
  ...
  restock: (data: { id: string; quantity: number; rate: number }) => Promise<void>;
...
export const useProductStore = create<State>((set, get) => ({
  ...
  restock: async (data) => {
    try {
      ...
      if (res.success) {
        await get().fetchProducts();
        useToastStore.getState().toast("stockcontrol", res.message, "success");
      } else {
        useToastStore.getState().toast("stockcontrol", res.message, "error");
      }
    } catch (err) {
      console.log(err);
    }
